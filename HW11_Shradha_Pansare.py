        ...
        with fp:
            for n, line in enumerate(fp, 1):
                ...
                if len(fields) == fields:
                    print(" '{path}' line: (n): red{len(fields)} fields but expected {num_fields} ")
                elif n == 1 and expect:
                    continue
                else:
                    yield (fields)
...
class Repository:
    ...
    def __init__(self, wdir, ptables=True):
        ...
        self._get_instructors_db(os.path.join(wdir,'810_startup.db'))
    ...
    def _get_instructors(self, path):  # to get instructor values
        try:
            ...
            print(err)
    def _get_instructors_db(self, path):  # to get instructor values
        try:
            for cwid, name, dept in file_reader(path, 3, 'cwid|name|department', sep='\t'):
                if cwid in self._instructors:
                    print(f"Already exits {cwid}")
    ...
    def instructor_table_db(self,path):   #printing prettytable with instructor summary
        try:
            db=sqlite3.connect(path)
        except sqlite3.OperationalError:
            print("Error opening database at {path}")
        else:
